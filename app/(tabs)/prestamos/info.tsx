...
export default function InfoValeScreen() {
  ...
  // onAuthStateChanged garantiza que Firebase ya restauró la sesión
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchVale = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          setVale({ id: docSnap.id, ...docSnap.data() });
        } else {
  ...
  // ─── Confirmar entrega (solo admin, irreversible) ────────────────────────────
  const handleConfirmarEntrega = async () => {
    if (vale?.entregaConfirmada) return;

    // id puede llegar como string o array desde useLocalSearchParams
    const docId = Array.isArray(id) ? id[0] : (id as string);
    if (!docId) {
      console.error("ID del vale no disponible");
      return;
    }

    // En web Alert.alert no muestra diálogo — usamos confirm() nativo del navegador
    const confirmed =
      Platform.OS === "web"
        ? window.confirm(
            `¿Confirmar entrega del Vale #${vale?.numeroFormulario}?\n\nEsta acción no se puede revertir.`,
          )
    ...
    setConfirmando(true);
    try {
      const docRef = doc(firestore, "prestamos", docId);
      await updateDoc(docRef, {
        entregaConfirmada: true,
        entregaConfirmadaAt: new Date().toISOString(),
        entregaConfirmadaPor: currentUserEmail ?? "admin",
      });
      setVale((prev: any) => ({
        ...prev,
        entregaConfirmada: true,
      ...
      if (Platform.OS === "web") {
        window.alert("✅ Entrega confirmada correctamente.");
      } else {
        Alert.alert("✅ Listo", "El vale ha sido marcado como entregado.");
      }
    } catch (error: any) {
      console.error("Error confirmando entrega:", error);
      if (Platform.OS === "web") {
        window.alert("Error: " + (error?.message ?? "No se pudo confirmar."));
      } else {
        Alert.alert(
          "Error",
      ...
      <html>
        ...
        <head>
          ...
          <style>
            ...
            height: 5.5in; padding: 0.5in; box-sizing: border-box;
        ...
        <View style={styles.headerApp}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <ThemedText style={styles.dateText}>
              {vale?.createdAt
                ? new Date(vale.createdAt).toLocaleDateString()
                : ""}
            </ThemedText>
          </View>
          ...
          <Pressable onPress={handleDownloadPDF} style={styles.downloadButton}>
            ...
            {isDesktop && (
              <ThemedText
                style={{ color: "#1976D2", marginLeft: 8, fontWeight: "bold" }}
              >
                Descargar PDF
              </ThemedText>
            )}
          ...
          <View style={isDesktop ? styles.desktopRow : null}>
            <View style={isDesktop ? styles.desktopColumn : null}>
              ...
              >
                ...
                <InfoBlock
                  label="Paciente"
                  value={vale?.nombrePaciente}
                  icon="person-outline"
                />

                <InfoBlock
                  label="Habitación"
                  value={vale?.habitacion}
                  icon="bed-outline"
                />
              </View>
            ...
            <View
              style={
                isDesktop ? [styles.desktopColumn, { marginLeft: 20 }] : null
              }
            >
              <ThemedText type="defaultSemiBold" style={styles.sectionTitle}>
                Insumos
              ...
              <View
                style={[
                  ...
                  isDesktop && styles.firmaContainerPC,
                ]}
              >
                <View style={styles.firmaBox}>
                  ...
                  <ThemedText style={styles.nombreFirma}>
              ...
              {/* ─── Checkbox Confirmar Entrega — Solo visible para admin ─── */}
              {isAdmin && (
                <View style={styles.confirmSection}>
                  ...
                  <Pressable
                    onPress={
                      vale?.entregaConfirmada
                        ? undefined
                        : handleConfirmarEntrega
                    }
                    disabled={confirmando || vale?.entregaConfirmada}
                    style={[
                      styles.checkboxRow,
                      vale?.entregaConfirmada
                        ? {
                  ...
                  >
                    {confirmando ? (
                      ...
                      <Ionicons
                        ...
                        size={26}
                        color={vale?.entregaConfirmada ? "#66BB6A" : "#1976D2"}
                        style={{ marginRight: 12 }}
                      />
                    )}
                    <View style={{ flex: 1 }}>
                      ...
                      {vale?.entregaConfirmada && vale?.entregaConfirmadaAt ? (
                        <ThemedText
                          style={[styles.checkboxMeta, { color: "#81C784" }]}
                        >
                          {new Date(vale.entregaConfirmadaAt).toLocaleString(
                            "es-SV",
                          )}{" "}
                          · {vale.entregaConfirmadaPor}
                        </ThemedText>
                      ) : (
                        <ThemedText style={styles.checkboxMeta}>
...
const styles = StyleSheet.create({
  ...
  // ─── Estilos del bloque de confirmación ───────────────────────────────────
